...
namespace Trabalho_AED2
{
       ...
       private void verificarPalavrasToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        {
            using (FormAdicionarPalavra formAdicionarPalavra = new FormAdicionarPalavra())
            {
                if (formAdicionarPalavra.ShowDialog() == DialogResult.OK)
                {
                    string? novaPalavra = formAdicionarPalavra.Palavra;
                    if (!string.IsNullOrWhiteSpace(novaPalavra))
                    {
                        editor.AdicionarPalavraAoDicionario(novaPalavra);
                        editor.SalvarDicionario("dicionario.txt");
                        MessageBox.Show($"A palavra '{novaPalavra}' foi adicionada ao dicionário.", "Palavra Adicionada", MessageBoxButtons.OK);
                    }
                }
            }
        }
